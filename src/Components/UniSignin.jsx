...
console.log('formData being sent:', formData);
    ...
    try {
      ...
      const response = await axios.post('api/uni/signin', formData, {
        headers:{
  ...
  console.log('Request completed');
      ...
      if (token) {
        ...
        if (userRoles.includes('university')) {
          const redirectTo = location.state?.from || '/universityportal';
          navigate(redirectTo);
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 text-center flex flex-col items-center justify-center p-8">
      <h1 className=" text-2xl md:text-4xl font-bold mb-4">Ready To Shape Tomorrow's Educators</h1>
      ...
      />
        ...
        <div className="mt-6 text-center">
          <p className="text-sm">
            ...
            <Link to="/unisignup" className="text-blue-600 hover:underline">
              Sign Up
            </Link>
          </p>
        </div>
